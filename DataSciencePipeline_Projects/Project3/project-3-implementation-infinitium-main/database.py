...
def get_by_date(start_date, end_date, collection, source):
    ...
    if source == 'twitter':
        ...
        try:
            ...
            return None
        doc = youtube_date(start_date, end, collection)
        for x in doc:
            d = x['_id']['new_date']
            v = x['count']
            date_count[d] = v
...
def twitter_date(start_date, end_date, collection):
    return collection.aggregate([{"$project": {"date": {"$dateFromString": {"dateString": "$time"}}}},
...
def youtube_date(start_date, end_date, collection):
    return collection.aggregate([{"$project": {"date": {"$dateFromString": {"dateString": "$time"}}}},
                                 {"$project": {"date": {"day": {"$dayOfMonth": '$date'}, "month": {"$month": "$date"},
                                                        "year": {"$year": '$date'}}}},
                                 {"$project": {"new_date": {
